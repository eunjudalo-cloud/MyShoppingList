...
# --- 데이터 저장 (접속한 IP별로 각각 다른 파일에 보관) ---
DATA_DIR = Path(__file__).parent / "data"
DATA_DIR.mkdir(exist_ok=True)


def client_key() -> str:
    """접속자를 구분하는 키. 클라이언트 IP를 해시해서 사용한다.
    (IP를 못 얻으면 'local' 하나로 묶임 — 로컬 실행 시엔 보통 같은 값)"""
    ip = getattr(st.context, "ip_address", None) or "local"
    return hashlib.sha256(ip.encode("utf-8")).hexdigest()[:16]


def data_file() -> Path:
    return DATA_DIR / f"{client_key()}.json"


def load_data() -> dict:
    """이 접속자의 쇼핑 리스트를 불러온다. 없거나 손상됐으면 빈 리스트."""
    try:
